...
export default function GameRoom() {
  ...
  // Fetch room data when user is available
  useEffect(() => {
    if (user) {
      fetchRoomData()
    }
  ...
  // Determine current turn when playing
  useEffect(() => {
    ...
    return (
      <main className="min-h-screen bg-pink-50">
        ...
        <div className="max-w-6xl mx-auto px-4 py-12 text-center">
          ...
          <Link href="/pass-the-brush" className="text-pink-500 hover:underline">
            ← Back to Hub
          </Link>
        </div>
      </main>
                  ...
                  <div className="flex-1">
                    <p className="font-medium text-gray-900">
                      ...
                      {player.user_id === room.host_id && (
                        <span className="ml-2 text-xs bg-pink-100 text-pink-600 px-2 py-1 rounded">HOST</span>
                      )}
                    </p>
  ...
  if (room.status === 'topic_selection') {
    ...
    return (
      <main className="min-h-screen bg-pink-50">
        ...
        <div className="max-w-4xl mx-auto px-4 py-12">
          {isPicker ? (
            <TopicPicker room={room} supabase={supabase} roomCode={roomCode} />
          ) : (
            <div className="bg-white rounded-lg shadow-lg p-12 text-center">
              <div className="text-6xl mb-6">⏳</div>
  ...
  return (
    <main className="min-h-screen bg-pink-50">
      <Header />
      <div className="max-w-6xl mx-auto px-4 py-12 text-center">
        <p className="text-gray-600">Loading game... (Status: {room.status})</p>
      </div>
    </main>
  )
}